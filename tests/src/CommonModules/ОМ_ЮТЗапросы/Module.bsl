...
Процедура Записи_Условие_ВСписке() Экспорт
	...
	// Создаем тестовые товары с уникальными именами для изоляции теста
	Товар = ЮТест.Данные().КонструкторОбъекта("Справочники.Товары")
		.Фикция("Наименование")
		.Фикция("Поставщик")
		.Установить("Вид", ВидТовар)
		.Записать();
	
	Услуга = ЮТест.Данные().КонструкторОбъекта("Справочники.Товары")
		.Фикция("Наименование")
		.Фикция("Поставщик")
		.Установить("Вид", ВидУслуга)
		.Записать();
	
	// Тест 1: Поиск по двум видам товаров
	Результат = ЮТЗапросы.Записи(ИмяТаблицы, ЮТест.Предикат()
		.Свойство("Вид").ВСписке(ЮТКоллекции.ЗначениеВМассиве(ВидТовар, ВидУслуга))
		.Свойство("Ссылка").ВСписке(ЮТКоллекции.ЗначениеВМассиве(Товар, Услуга)));
	
	ЮТест.ОжидаетЧто(Результат, "Поиск по двум видам товаров")
		.ИмеетДлину(2)
		.ЛюбойЭлементСодержитСвойствоСоЗначением("Ссылка", Товар)
		.ЛюбойЭлементСодержитСвойствоСоЗначением("Ссылка", Услуга);
	
	// Тест 2: Поиск только по виду "Товар" среди созданных нами товаров
	Результат = ЮТЗапросы.Записи(ИмяТаблицы, ЮТест.Предикат()
		.Свойство("Вид").Равно(ВидТовар)
		.Свойство("Ссылка").Равно(Товар));
	
	ЮТест.ОжидаетЧто(Результат, "Поиск товара по виду")
		.ИмеетДлину(1)
		.Свойство("[0].Ссылка").Равно(Товар);
	
	// Тест 3: Поиск только по виду "Услуга" среди созданных нами товаров
	Результат = ЮТЗапросы.Записи(ИмяТаблицы, ЮТест.Предикат()
		.Свойство("Вид").Равно(ВидУслуга)
		.Свойство("Ссылка").Равно(Услуга));
	
	ЮТест.ОжидаетЧто(Результат, "Поиск услуги по виду")
		.ИмеетДлину(1)
		.Свойство("[0].Ссылка").Равно(Услуга);
