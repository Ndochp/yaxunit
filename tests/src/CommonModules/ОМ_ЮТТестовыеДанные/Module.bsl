...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты.Тег("ТестовыеДанные").УдалениеТестовыхДанных()
		...
		.ДобавитьТест("ФиктивныйУникальныйИдентификатор")
		.ДобавитьТест("ЗагрузитьИзМакета_КолонкаСоставногоТипаИзКэша")
	;
...
#Если Сервер Тогда
		Ютест.ОжидаетЧто(ТаблицаРезультатов, Вариант.Описание)
			...
			.Свойство("[3].Товар.Наименование").Равно("Набор")
			.Свойство("[3].Количество").Равно(2)
			.Свойство("[3].Цена").Равно(3000)
			.Свойство("[3].Сумма").Равно(6000)
		;
#Иначе
		Ютест.ОжидаетЧто(ТаблицаРезультатов, Вариант.Описание)
			.ИмеетТип("Массив")
...
Процедура ФиктивныйУникальныйИдентификатор() Экспорт
	
	// Проверка генерации через прямой тип
	Значение1 = ЮТест.Данные().Фикция(Тип("УникальныйИдентификатор"));
	ЮТест.ОжидаетЧто(Значение1)
		.ИмеетТип("УникальныйИдентификатор")
		.Заполнено();
	
	// Проверка генерации через описание типов
	ОписаниеТипа = Новый ОписаниеТипов("УникальныйИдентификатор");
	Значение2 = ЮТест.Данные().Фикция(ОписаниеТипа);
	ЮТест.ОжидаетЧто(Значение2)
		.ИмеетТип("УникальныйИдентификатор")
		.Заполнено();
	
	// Проверка уникальности генерируемых значений
	ЮТест.ОжидаетЧто(Значение1)
		.НеРавно(Значение2);
	
КонецПроцедуры

Процедура ЗагрузитьИзМакета_КолонкаСоставногоТипаИзКэша() Экспорт
	...
	ОписанияТипов = Новый Соответствие;
	ОписанияТипов.Вставить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
	...
	РанееСозданныйКонтрагент = ТаблицаЗначений[0].Контрагент;
	...
	ЮТест.ОжидаетЧто(ТаблицаРезультатов)
		.ИмеетТип("ТаблицаЗначений")
		.ИмеетДлину(2)
		.Свойство("[0].Аналитика").ИмеетТип("СправочникСсылка.Контрагенты").Равно(РанееСозданныйКонтрагент)
