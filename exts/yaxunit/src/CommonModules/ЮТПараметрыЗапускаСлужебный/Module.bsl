...
Функция ПреобразоватьЗначениеПараметра(ЗначениеСтрокой) Экспорт
	...
	Если СтрДлина(ЗначениеСтрокой) > 0 И СтрДлина(ЗначениеСтрокой) <= 20 Тогда
...
Функция ЭтоАбсолютныйПуть(Путь)
	...
	#Если НЕ ВебКлиент Тогда
		Если ЮТОкружение.ЭтоWindows() Тогда
			...
			Если СтрДлина(Путь) >= МинимальнаяДлинаПутиWindows() И Сред(Путь, ПозицияДвоеточияWindows(), 1) = ":" И Сред(Путь, ПозицияСлешаWindows(), 1) = "\" Тогда
				Возврат Истина;
			КонецЕсли;
			
			Если СтрДлина(Путь) >= МинимальнаяДлинаСетевогоПути() И Лев(Путь, МинимальнаяДлинаСетевогоПути()) = "\\" Тогда
				Возврат Истина;
			КонецЕсли;
		Иначе
