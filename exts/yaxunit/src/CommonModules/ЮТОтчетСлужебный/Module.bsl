...
Процедура СформироватьНесколькоОтчетов(РезультатВыполнения, ПараметрыЗапуска, Обработчик)
	...
	Для Каждого Элемент Из ПараметрыЗапуска.reports Цикл
		...
		ПараметрыФормирования = Новый Структура("РезультатВыполнения, ПараметрыЗапуска, Обработчик", РезультатВыполнения, ЛокальныеПараметрыЗапуска, Новый ОписаниеОповещения("ПослеГенерацииОдиночногоОтчета", ЭтотОбъект, Состояние));
		ПараметрыФормирования.Вставить("МодульОтчета", МодульОтчета);
		ПараметрыФормирования.Вставить("Формат", Формат);
...
// ПослеГенерацииОдиночногоОтчета
//  Оповещение по завершению формирования одного отчета в режиме множественных отчетов.
// Параметры:
//  НеИспользуется - Произвольный - Зарезервирован для совместимости цепочек вызовов
//  Состояние - Структура - Служебные параметры процесса (Осталось, ОригинальныйОбработчик)
Процедура ПослеГенерацииОдиночногоОтчета(НеИспользуется = Неопределено, Состояние) Экспорт
