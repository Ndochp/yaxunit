...
Функция ОписаниеОбъектаМетаданных(Знач Значение, ЗаполнятьРеквизиты = Истина) Экспорт
	...
	Если ОписаниеТипа.ИмяКоллекции = "РегистрыБухгалтерии" Тогда
		// Формирование описаний стандартных реквизитов регистра бухгалтерии:
+		// При наличии корреспонденции: СчетДт/СчетКт и парные реквизиты субконто (Дт/Кт)
+		// Без корреспонденции: единственный реквизит Счет
		...
		ВыдаватьОшибку = ПроверкаЗаполнения.ВыдаватьОшибку;
		КоллекцияРеквизитов = Новый Массив();
		Если МетаданныеОбъекта.Корреспонденция Тогда
			
			КоллекцияРеквизитов.Добавить(Новый Структура("Имя, Тип, ПроверкаЗаполнения", "СчетДт", ОписаниеТипаПланСчета, ВыдаватьОшибку));
			КоллекцияРеквизитов.Добавить(Новый Структура("Имя, Тип, ПроверкаЗаполнения", "СчетКт", ОписаниеТипаПланСчета, ВыдаватьОшибку));
			
			// добавление ВидСубконтоДт и удаление ВидСубконто
			МассивМаркеров = Новый Массив();
			МассивМаркеров.Добавить("ВидСубконто");
			МассивМаркеров.Добавить("Субконто");
			УдаляемыеОписания = Новый Массив();
			Для Каждого КлючЗначение Из ОписаниеОбъект.Реквизиты Цикл
				ОписаниеРеквизита = КлючЗначение.Значение;
				Для Каждого Элемент Из МассивМаркеров Цикл
					Если Не СтрНачинаетсяС(ОписаниеРеквизита.Имя, Элемент) Тогда Тогда
						Продолжить;
					КонецЕсли;
					Кт = СтрЗаменить(ОписаниеРеквизита.Имя, Элемент, Элемент + "Кт");
					Дт = СтрЗаменить(ОписаниеРеквизита.Имя, Элемент, Элемент + "Дт");
					Заполнение = Неопределено;
					Если ОписаниеРеквизита.Обязательный Тогда
						Заполнение = ПроверкаЗаполнения.ВыдаватьОшибку;
					КонецЕсли;
					КоллекцияРеквизитов.Добавить(Новый Структура("Имя, Тип, ПроверкаЗаполнения", Кт, ОписаниеРеквизита.Тип, Заполнение));
					КоллекцияРеквизитов.Добавить(Новый Структура("Имя, Тип, ПроверкаЗаполнения", Дт, ОписаниеРеквизита.Тип, Заполнение));
					УдаляемыеОписания.Добавить(КлючЗначение.Ключ);
				КонецЦикла;
			КонецЦикла;
			Для Каждого Элемент Из УдаляемыеОписания Цикл
				ОписаниеОбъект.Реквизиты.Удалить(Элемент);
			КонецЦикла;
		
		Иначе
			КоллекцияРеквизитов.Добавить(Новый Структура("Имя, Тип, ПроверкаЗаполнения", "Счет", ОписаниеТипаПланСчета, ВыдаватьОшибку));
		КонецЕсли;
		
		ДобавитьОписанияРеквизитов(КоллекцияРеквизитов, ОписаниеОбъект.Реквизиты, Ложь, Истина);
