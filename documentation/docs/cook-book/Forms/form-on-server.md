...
Основная проблема в таком тесте, это получить форму на сервере, т.к. создать форму на сервере не получится и с клиента её нельзя передать.
...
Работа нашего теста будет состоять из 3-х шагов:
```mermaid
sequenceDiagram
    participant Тест
    participant Форма
...
#Если Клиент Тогда // Шаг 1
    ...
    Форма.Закрыть();
#Иначе // Шаг 3
// в эту ветку мы попадаем уже после вызова тестового метода из обработчика модуля формы ПриСозданииНаСервере и имеем в своём распоряжении Форму
