...
При тестировании обменов или интеграций, использующих SOAP или XML, вам может пригодиться [`ЮТест.Данные().КонструкторОбъектаXDTO`](/api/ЮТТестовыеДанные#конструкторобъектаxdto). Этот инструмент позволяет легко создавать и заполнять объекты XDTO с поддержкой автоматической генерации данных. Он был создан на основании уже проверенного и зарекомендовавшего себя [`ЮТест.Данные().КонструкторОбъекта`](/api/ЮТТестовыеДанные#конструкторобъекта) и обладает схожим API.
...
* [`Установить(ИмяРеквизита, Значение)`](/api/ЮТКонструкторОбъектаXDTO#установить) - Устанавливает значение указанного реквизита объекта
* [`УстановитьРеквизиты(ЗначенияРеквизитов)`](/api/ЮТКонструкторОбъектаXDTO#установитьреквизиты) - Устанавливает значения реквизитов объекта из структуры
* [`Фикция(ИмяРеквизита)`](/api/ЮТКонструкторОбъектаXDTO#фикция) - Генерирует и устанавливает случайное значение для реквизита с учетом его типа:
...
* [`ФикцияРеквизитов(ИменаРеквизитов)`](/api/ЮТКонструкторОбъектаXDTO#фикцияреквизитов) - Генерирует случайные значения для списка реквизитов
* [`ФикцияОбязательныхПолей()`](/api/ЮТКонструкторОбъектаXDTO#фикцияобязательныхполей) - Заполняет все обязательные поля случайными значениями
* [`ФикцияНезаполненных()`](/api/ЮТКонструкторОбъектаXDTO#фикциянезаполненных) - Заполняет все незаполненные реквизиты случайными значениями
* [`ФикцияНезаполненныхИсключая(ИменаИсключений)`](/api/ЮТКонструкторОбъектаXDTO#фикциянезаполненныхисключая) - Заполняет все незаполненные реквизиты случайными значениями, кроме указанных
* [`ДобавитьНовый(ИмяРеквизита)`](/api/ЮТКонструкторОбъектаXDTO#добавитьновый) - Добавляет новый объект в коллекцию. Все последующие вызовы установки реквизитов будут относиться к этому добавленному объекту
* [`ПерейтиКВладельцу()`](/api/ЮТКонструкторОбъектаXDTO#перейтиквладельцу) - Возвращает контекст заполнения к родительскому объекту. Используется после метода `ДобавитьНовый` для переключения контекста заполнения на родительский объект
* [`ДанныеОбъекта()`](/api/ЮТКонструкторОбъектаXDTO#данныеобъекта) - Возвращает созданный объект XDTO
