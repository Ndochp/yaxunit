...
ТаблицаИсходныхДанных = ЮТест.Данные().ЗагрузитьИзМакета(
    "ОбщийМакет.ЮТ_МакетТестовыхДанных.R1C1:R4C4",
    ОписанияТипов,
    КэшЗначений
);
...
ТаблицаОжидаемыхЗначений = ЮТест.Данные().ЗагрузитьИзМакета(
    "ОбщийМакет.ЮТ_МакетТестовыхДанных.R5C1:R8C4", 
    ОписанияТипов,
    КэшЗначений
);
...
Можно использовать составные типы в колонках таблицы значений, но создаваться объекты метаданных для такой колонки будут по первому типу из списка указанных типов колонки.
Чтобы не создавать не нужные данные существует 2 варианта заполнения колонки с составным типом:

1. Использовать значение из кэша, когда там есть значение с совпадающим типом и идентификатором.
   
```bsl
КэшЗначений = Неопределено;
	
МассивТиповАналитики = Новый Массив;	
МассивТиповАналитики.Добавить(Тип("СправочникСсылка.Склады"));
МассивТиповАналитики.Добавить(Тип("СправочникСсылка.Контрагенты"));
	
ОписанияТипов = Новый Соответствие;
ОписанияТипов.Вставить("Аналитика", Новый ОписаниеТипов(МассивТиповАналитики));
ОписанияТипов.Вставить("Поставщик", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
...
ИсходныеДанные = Новый ТекстовыйДокумент;
ИсходныеДанные.УстановитьТекст(Текст);
	
ТаблицаПоставщиков = ЮТест.Данные().ЗагрузитьИзМакета(
   ИсходныеДанные.ПолучитьОбласть("Поставщики"),
   ОписанияТипов,
   КэшЗначений
);
	
Поставщик = ТаблицаПоставщиков[0].Поставщик;
	
// Вызов тестируемого сценария
	
ТаблицаРезультатов = ЮТест.Данные().ЗагрузитьИзМакета(
   ИсходныеДанные.ПолучитьОбласть("Аналитики"),
   ОписанияТипов,
   КэшЗначений
);
...
ЮТест.ОжидаетЧто(ТаблицаРезультатов)
   .ИмеетТип("ТаблицаЗначений")
   .ИмеетДлину(2)
   .Свойство("[0].Аналитика").ИмеетТип("СправочникСсылка.Контрагенты").Равно(Поставщик)
   .Свойство("[1].Аналитика").ИмеетТип("СправочникСсылка.Склады").Заполнено();
```

2. Можно воспользоваться соответствием "ЗаменяемыеЗначения".
```bsl
...
ЗаменяемыеЗначения.Вставить("Аналитика (Склад)", Склад);

ИсходныеДанные = "| Аналитика             |
                 ||-----------------------|
                 || Аналитика (Поставщик) |
                 || Аналитика (Склад)     |";
...
Данные макетов нередко применяются для создания тестового окружения - загрузки данных в информационную базу.
