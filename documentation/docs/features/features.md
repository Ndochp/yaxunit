...
То есть заменить какой-либо объект обманкой, упрощенной его версией.
...
* [`Мокито`](mocking/mockito/mockito.md) позволяет настроить/изменить поведение нужных методов конфигурации
  ...
  * [`ВыброситьИсключение`](/api/МокитоОбучение#выброситьисключение) в нужный момент.
  ...
  * За настройку условий срабатывания тех или иных действий отвечает метод [`Когда`](/api/МокитоОбучение#когда)
...
* Если метод должен изменить данные в информационной базе, то эти изменения можно проверить с помощью [утверждений ИБ](assertions/assertions-db), [описании API утверждений](/api/ЮТУтвержденияИБ).
...
* [`ЮТест.Упал`](/api/ЮТест#упал) - Вызывает явное падение теста с возможностью указать сообщение и значения для сравнения
