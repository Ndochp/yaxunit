...
* Параметризированные тесты являются предпочитаемым вариантом
  * Для каждой комбинации значений параметров формируется отдельная запись в отчете, в котором указываются использованные значения параметров и результаты проверки.
