...
* Если по какой-то причине не удалось разрешить зависимость, то зависимые тесты не будут выполняться.
...
В примере движок проверит наличие файла `.gitignore` в каталоге проекта и перенесет его на сервер и сохранит в контексте путь к файлу (причем, для сервера и клиента будут сохранены разные пути).
...
Эта зависимость переносит файлы на сервер, чтобы они были доступны в серверных тестах и сохраняет в контексте путь к перенесенному файлу/каталогу (для клиентского теста будет храниться путь к файлам на клиенте).
